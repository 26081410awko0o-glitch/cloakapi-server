...
تشمل: التحقق من الـ API Keys، Rate Limiting، وإدارة الجلسات مع التخزين الدائم.
"""
        ...
        with self._lock:
            ...
            if current_count >= self.max_requests:
                ...
                return False, {
                    ...
                    "remaining": 0,
                    "reset_seconds": max(reset_time, 1),
...
class APIKeyManager:
    """
    مدير مفاتيح الـ API مع التخزين الدائم في ملف JSON.
    """
    def __init__(self):
        self.file_path = DATA_DIR / "keys.json"
        self._lock = Lock()
    ...
    def _save_keys(self):
        self.file_path.write_text(json.dumps(self._keys, indent=2, ensure_ascii=False), encoding="utf-8")
...
class CodeVault:
    """
    خزنة الأكواد مع التخزين الدائم في ملف JSON.
    """
    def __init__(self):
        self.file_path = DATA_DIR / "vault.json"
        self._lock = Lock()
    ...
    def exists(self, endpoint_id: str) -> bool:
        with self._lock:
            return endpoint_id in self._vault
